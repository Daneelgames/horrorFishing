...
public class GameManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (player && player.health > 0 && (player.pm.controller.enabled || (!player.pm.controller.enabled && player.pm.inTransport)) && (!lg || !lg.generating))
        {
            ...
            if (!ui || paused || mementoWindow || tutorialPassed != 1 && hub) return;
    ...
    public IEnumerator StartGame()
    {
        ...
        {
            AOfield = SceneManager.LoadSceneAsync(1, LoadSceneMode.Additive); // load intro
            AOfield.allowSceneActivation = false;
        }
        ...
        else if (hub)
        {
            ...
            if (HubItemsSpawner.instance)
                HubItemsSpawner.instance.SpawnItems();
    ...
    public void LoadGame()
    {
        ...
        if (data != null)
        {
            ...
            if (data.heartControllersTakenIndexesInHub != null)
                heartControllersTakenIndexesInHub = new List<int>(data.heartControllersTakenIndexesInHub);
    ...
    public void DeleteLocalSave(bool deathOnMeatzone)
    {
        ...
        heartControllersTakenIndexesInHub.Clear();
