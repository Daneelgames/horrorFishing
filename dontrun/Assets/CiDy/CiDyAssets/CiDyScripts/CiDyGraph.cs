...
public class CiDyGraph : MonoBehaviour {
    ...
    */
	...
	void GrabActiveTerrains() {
		if (terrains == null){
			//Do Not Overwrite Previous Terrain Data.
			//This is the First Time we are Grabbing Terrain Data.
			Terrain[] currentTerrains = Resources.FindObjectsOfTypeAll(typeof(Terrain)) as Terrain[];//Get All Active Terrains
			//Sort Terrains By X and Z Axis, 
			currentTerrains = currentTerrains.OrderBy(x => x.GetPosition().x).ThenBy(x => x.GetPosition().z).ToArray();
			terrains = new StoredTerrain[currentTerrains.Length];
			for (int i = 0; i < currentTerrains.Length; i++)
			{
				//Create StoredTerrain Data Set.
				terrains[i] = new StoredTerrain(i, currentTerrains[i]);
			}
...
#if GAIA_2_PRESENT || GAIA_PRO_PRESENT
		biomeSpawner = FindObjectOfType<BiomeController>();
#endif
...
/*#if UNITY_EDITOR
    ...
    */
    ...
    //This function will drop the Points to the Terrains Heights
    public void ContourPathToTerrain(ref Vector3[] path, int[] blendingTerrains)
    {
		if (terrains == null || blendingTerrains == null || terrains.Length == 0 || blendingTerrains.Length == 0) {
		...
		for (int i = 0; i < path.Length; i++)
		{
			...
			for(int t = 0; t < blendingTerrains.Length; t++)
            {
				...
				if (terrBounds.Contains(pos))
                {
					pos.y = terrains[terrainIdx]._Terrain.SampleHeight(pos);
	...
	public IEnumerator BlendRoads(int terrainId){
		...
		for(int i = 0;i<roads.Count;i++){
            ...
            //Call Blend for this RoadMesh
            //BlendRoad(roadMesh, roads[i].GetComponent<CiDyRoad>().width,radius,zBuffer,edgeOffset,smoothBorder);
            BlendRoad(terrainId, roads[i], Mathf.RoundToInt(smoothBorder));
			curProblems--;
            #if UNITY_EDITOR
                float percentage = Mathf.Round(((float)i/(float)roads.Count) * 100);
	...
	public IEnumerator BlendNodes(int terrainIdx){
        ...
        if (masterGraph.Count>1){
            ...
            for (int i = 0;i<masterGraph.Count;i++){
                //Is this Node Blending to this terrain?
                if (!masterGraph[i].MatchTerrain(terrainIdx))
                {
					//This node is not blending on this terrain, skip it.
					continue;
                }
                ...
                if (masterGraph[i].type == CiDyNode.IntersectionType.continuedSection)
                {
                    ...
                    if (nodeMesh != null)
                    {
                        ...
                        for (int j = 0; j < nodeMesh.vertices.Length - 2; j += 2)
                        {
                            ...
                            BlendTerrainOfRoad(terrainIdx,terrPos, tSize, mapScale, nodeMesh.vertices[j]+masterGraph[i].position, nodeMesh.vertices[j + 1] + masterGraph[i].position, nodeMesh.vertices[j + 2] + masterGraph[i].position, nodeMesh.vertices[j + 3] + masterGraph[i].position, roadRadius, roadMask2, terrains[terrainIdx].terrHeights.hmRes);
                        }
                    }
                }
                else
...
#if GAIA_2_PRESENT || GAIA_PRO_PRESENT
	private BiomeController biomeSpawner;//Need for checking GAIA presents in Scene.
#endif
	...
	public IEnumerator UpdateTerrainDetails() {
		...
		if (biomeSpawner)
		{
			...
			for (int i = 0; i < spawners.Length; i++)
			{
				...
				UnityEditor.EditorUtility.DisplayProgressBar("Masking GAIA:", " GAIA Spawners: " + percentage + "% " + "(" + i + "/" + totalProblems + ")", (1.0f - ((float)curProblems / (float)totalProblems)));
#endif
				...
				//Add Image Mask
				SpawnerSettings settings = spawners[i].m_settings;
				List<ImageMask> images = settings.m_imageMasks.ToList();
				if (spawners[i].name.Contains("Grass") || spawners[i].name.Contains("Fern"))
				{
					...
					for (int j = 0; j < images.Count; j++)
					{
						...
						{
							if (images[j].ImageMaskTexture.name.Contains("Grass_" + curMaskTextureName) || images[j].ImageMaskTexture.name == "")
							{
								//Remove it
								images.RemoveAt(j);
								break;
	...
	void BlendTerrainVegetation(int terrainIndex) {
		//Debug.Log("Blend Terrain Vegetation");
...
//Set Tree Instances of Terrain to Match Updated tree List
    ...
    void BlendRoad(int terrainIdx, GameObject road, int smoothBorder)
    {
	...
	//Updated Blend Terrain At Point
	// Set all Terrain Heights to just under our Position in a Heights map that are under the Position in Space and Radius
	void BlendTerrainOfRoad(int terrainId, Vector3 terrPos, Vector3 tSize, Vector3 mapScale, Vector3 pA, Vector3 pB, Vector3 pC, Vector3 pD, int radius, int mask, int mRes)
    {
...
public class StoredTerrain
{
	...
	public void ReAddGrass() {
		int[] layers = terrData.GetSupportedLayers(0, 0, terrData.detailWidth, terrData.detailHeight);
		//Debug.Log(" Total Layers: "+ layers.Length+", Terrain Grass: "+terrainGrass.Length);
	...
	public Bounds ReturnBounds() {
		return new Bounds(terrData.bounds.center + _Terrain.transform.position, terrData.bounds.size);
	}
}
