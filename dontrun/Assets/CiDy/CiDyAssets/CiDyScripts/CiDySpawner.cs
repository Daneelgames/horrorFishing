...
public class CiDySpawner : MonoBehaviour
{
    ...
    public void SetPath(Vector3[] newPath, float width, Vector3 _startRightDir, Vector3 _endRightDir, bool regenerate = false) {
        spline = newPath;
        roadWidth = width;
    ...
    public void Generate()
    {
        ...
        //Offset Path
        normalizedSplineRange = OffsetPath(normalizedSplineRange, pathOffset, ref _startDir, ref _endDir, reverseDirection, setToGround);
        ...
        switch (spawnType) {
            ...
            case SpawnerType.RoadMarkings:
                ...
                GenerateRoadMarkingMesh(normalizedSplineRange, _startDir, _endDir);
                break;
    ...
    void GenerateRoadMarkingMesh(Vector3[] path, Vector3 _startDir = default, Vector3 _endDir = default)
    {
    ...
    Vector3[] OffsetPath(Vector3[] spline, float offset, ref Vector3 startDir, ref Vector3 endDir, bool reverseFacing = false, bool placeOnGround = false) {
        ...
        for (int i = 0; i < spline.Length; i++)
        {
            ...
            dir.y = 0;
            //Get Right
            Vector3 right = Vector3.Cross(Vector3.up, dir);//Right by Default
    ...
    void PlacePrefab(ref List<GameObject> objectArray, GameObject prefab, Vector3 pos, Vector3 fwd, Vector3 up, bool worldUp = false) {
        ...
        {
            fwd.y = 0;
            newObj = Instantiate(prefab, pos, Quaternion.LookRotation(fwd, Vector3.up) * Quaternion.Euler(rotationCorrection.x, rotationCorrection.y, rotationCorrection.z), transform);
        }
        else
        {
            newObj = Instantiate(prefab, pos, Quaternion.LookRotation(fwd, up) * Quaternion.Euler(rotationCorrection.x, rotationCorrection.y, rotationCorrection.z), transform);
        }
