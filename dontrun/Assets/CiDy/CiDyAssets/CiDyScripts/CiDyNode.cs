...
public class CiDyNode : ScriptableObject {
    ...
    void CreateConnector() {
        ...
        if (leftPoints.Count == 2 && rightPoints.Count == 2)
        {
            ...
            //Set Road Material
            mRenderer.sharedMaterial = roadMaterial;
			GameObject markingsObj;
			Mesh markingMesh = new Mesh();
			if (markingObject == null)
			{
				markingsObj = new GameObject("Markings");
				markingObject = markingsObj;
				markingsObj.transform.SetParent(intersection.transform);
				markingsRenderer = markingsObj.AddComponent<MeshRenderer>();
				markingsRenderer.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
				markingsFilter = markingsObj.AddComponent<MeshFilter>();
				markingsRenderer.sharedMaterial = connectorMaterial;
			}
			else {
				markingsObj = markingObject;
				markingsRenderer = markingsObj.GetComponent<MeshRenderer>();
				markingsFilter = markingsObj.GetComponent<MeshFilter>();
			}
			//Reposition
			markingsObj.transform.position = intersection.transform.position;
			//Move Up
			markingObject.transform.position += (Vector3.up * 0.05f);
			//Create Markings Mesh
			markingMesh = CiDyUtils.CloneMesh(roadMesh);
			markingsFilter.sharedMesh = markingMesh;
        }
