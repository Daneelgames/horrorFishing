...
public class CiDyCell : MonoBehaviour {
	...
	void GetThemeLights() {
		pathLight = (GameObject)Resources.Load("CiDyResources/CiDyTheme" + graph.districtTheme[curType] + "/" + graph.districtTheme[curType] + "StreetLight/StreetLight", typeof(GameObject));
	}
...
#elif !UNITY_EDITOR
						GameObject placedBuilding = Instantiate(prefabBuildings[pickedPrefab], Vector3.zero, Quaternion.identity);
#endif
						//Prefab Bounds
...
#elif !UNITY_EDITOR
						GameObject newBuilding = Instantiate(prefabBuildings[bestFit], Vector3.zero, Quaternion.identity);
#endif
						//Prefab Bounds
                    ...
                    for (int t = 0; t < cidyLots[i].roadSides.Count; t++)
                    {
                        ...
                        while (remainingRoadAccess > 0)
                        {
                            ...
                            if (prefabWidths[pickedPrefab] <= remainingRoadAccess)
                            {
                                ...
                                if (!CiDyUtils.BoundsIntersect(boundFootPrint, buildings.ToArray()))// && !CiDyUtils.BoundsIntersect(boundFootPrint, lotBoundary))
                                {
    ...
    void CreateSideWalks()
    {
        ...
        lotPoly = insetNodes;
		//TODO Customer Request. Expose Enum for Vegetation Studio Biome Masks. Allowing the User to Choose the Enum of a Cells entire Biome Mask.
		//Create Vegitation Mask
#if VEGETATION_STUDIO_PRO || VEGETATION_STUDIO
            //Generate SideWalk Vegitation Masks.
